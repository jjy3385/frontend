...
const Particles = ({ isHovered }: ParticlesProps) => {
  ...
  const { initialPosTexture, targetDataTexture, geometry } = useMemo(() => {
    ...
    const GRAVITY_RADIUS_SQ = 6.0 * 6.0; 
    // ==================================================================================
    ...
    const cols = 80; const rows = 50;
    const width = 24; const height = 13;
    ...
    for (let i = 0; i < count; i++) {
      ...
      initialPosData[i * 4 + 0] = x; initialPosData[i * 4 + 1] = y; 
      initialPosData[i * 4 + 2] = z; initialPosData[i * 4 + 3] = 1.0; 

      targetData[i * 4 + 0] = x; targetData[i * 4 + 1] = y;
      targetData[i * 4 + 2] = z; targetData[i * 4 + 3] = 0.0; 

      const texX = (i % textureSize) / textureSize + (0.5 / textureSize);
      const texY = Math.floor(i / textureSize) / textureSize + (0.5 / textureSize);
      uvs[i * 2 + 0] = texX; uvs[i * 2 + 1] = texY;
      positions[i * 3] = 0; 
    }
    ...
    borderSeats.forEach(seat => {
        let bestIdx = -1; let minDistSq = Infinity;
        for(let i=0; i<count; i++) {
    ...
    for(let i=0; i<count; i++) {
         if (usedIndices.has(i)) continue; 
         const px = tempInitialPositions[i].x; const py = tempInitialPositions[i].y;
         
         // 중력 우물 범위 체크
         if (px*px + py*py < GRAVITY_RADIUS_SQ) {
             let minDistSq = Infinity; let bestSeat = null;
             // 대충 찾기 (step 10)
             for(let b=0; b<borderSeats.length; b+=10) {
                 const seat = borderSeats[b];
                 const dx = px - seat.x; const dy = py - seat.y;
                 const dSq = dx*dx + dy*dy;
                 if(dSq < minDistSq) { minDistSq = dSq; bestSeat = seat; }
             }
             if (bestSeat) {
                 targetData[i*4 + 0] = bestSeat.x; targetData[i*4 + 1] = bestSeat.y; targetData[i*4 + 2] = 0.0;
             }
         }
    }

    const initTex = new THREE.DataTexture(initialPosData, textureSize, textureSize, THREE.RGBAFormat, THREE.FloatType);
    initTex.needsUpdate = true;
    const targetTex = new THREE.DataTexture(targetData, textureSize, textureSize, THREE.RGBAFormat, THREE.FloatType);
